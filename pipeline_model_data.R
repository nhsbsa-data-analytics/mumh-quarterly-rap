...
#altered to add ageband and gender for 2022/23 Q3 covid model
#for later publications use start = max_month_plus_dw from above comment
#and append to previous data in shared area
create_tdim(con)  %>%
 compute("MUMH_MONTH_TDIM_dec22", analyze = FALSE, temporary = FALSE)
...
create_org_dim(con, country = 1) %>%
  compute("MUMH_MONTH_PORG_DIM_dec22", analyze = FALSE, temporary = FALSE)
...
create_drug_dim(con, bnf_codes = c("0401", "0402", "0403", "0404", "0411"))  %>%
  compute("MUMH_MONTH_DRUG_DIM_dec22", analyze = FALSE, temporary = FALSE)
...
fact %>%
  inner_join(tbl(con, "MUMH_MONTH_TDIM_dec22") , by = "YEAR_MONTH") %>%
  inner_join(tbl(con, "MUMH_MONTH_PORG_DIM_dec22") , by = c("PRESC_TYPE_PRNT" = "LVL_5_OUPDT",
                                                      "PRESC_ID_PRNT" = "LVL_5_OU")) %>%
  inner_join(tbl(con, "MUMH_MONTH_DRUG_DIM_dec22"), by = c("CALC_PREC_DRUG_RECORD_ID" = "RECORD_ID",
                                                     "YEAR_MONTH" = "YEAR_MONTH")) %>%
  inner_join(age,
             by = c("CALC_AGE" = "AGE")) %>%
  compute("MUMH_RAW_DATA_dec22", analyze = FALSE, temporary = FALSE)

## build and collect quarterly data
mumh_quarterly_dec22 <- tbl(con, "MUMH_RAW_DATA_dec22") %>%
  group_by(
    FINANCIAL_YEAR,
    FINANCIAL_QUARTER,
...
mumh_monthly_dec22 <- tbl(con, "MUMH_RAW_DATA_dec22") %>%
  group_by(
    FINANCIAL_YEAR,
    FINANCIAL_QUARTER,
    YEAR_MONTH,
...
mumh_model_dec22 <- tbl(con, "MUMH_RAW_DATA_dec22") %>%
  mutate(
...
save_data(mumh_quarterly_dec22, dir = "Y:/Official Stats/MUMH", filename = "mumh_quarterly")

save_data(mumh_monthly_dec22, dir = "Y:/Official Stats/MUMH", filename = "mumh_monthly")

save_data(mumh_model_dec22, dir = "Y:/Official Stats/MUMH", filename = "mumh_model")
}
...
#right align columns and round to 2 DP - C to AG (!!NEED TO UPDATE AS DATA EXPANDS!!)
format_data(wb,
            "Patient_Identification",
