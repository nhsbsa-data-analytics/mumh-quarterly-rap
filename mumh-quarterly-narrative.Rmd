...
raw_data$quarterly %>%
  ...
  filter(SECTION_CODE == "0403"& FINANCIAL_YEAR!="2015/2016") %>% 
  group_by(FINANCIAL_QUARTER, SECTION_NAME, SECTION_CODE) %>% 
  summarise(`Prescribed items` = sum(ITEM_COUNT),
            `Identified patients` = sum(PATIENT_COUNT),
            .groups = "drop") %>% 
  pivot_longer(cols = c(`Identified patients`,`Prescribed items`),
               names_to = "measure",
               values_to = "value") %>% 
...
chart_data$monthly %>% 
  ...
  filter(SECTION_CODE == "0403"& YEAR_MONTH>201603) %>% 
  rename(`Prescribed items` = ITEM_COUNT,
...
raw_data$quarterly %>%
  filter(SECTION_CODE == "0401"& FINANCIAL_YEAR!="2015/2016") %>% 
  group_by(FINANCIAL_QUARTER, SECTION_NAME, SECTION_CODE) %>% 
  summarise(`Prescribed items` = sum(ITEM_COUNT),
            `Identified patients` = sum(PATIENT_COUNT),
            .groups = "drop") %>% 
...
chart_data$monthly %>% 
  filter(SECTION_CODE == "0401"& YEAR_MONTH>201603) %>% 
  rename(`Prescribed items` = ITEM_COUNT,
         `Identified patients` = PATIENT_COUNT) %>% 
  pivot_longer(cols = c(`Identified patients`,`Prescribed items`),
               names_to = "measure",
...
raw_data$quarterly %>%
  filter(SECTION_CODE == "0402"& FINANCIAL_YEAR!="2015/2016") %>% 
  group_by(FINANCIAL_QUARTER, SECTION_NAME, SECTION_CODE) %>% 
  summarise(`Prescribed items` = sum(ITEM_COUNT),
            `Identified patients` = sum(PATIENT_COUNT),
            .groups = "drop") %>% 
...
raw_data$quarterly %>%
  filter(SECTION_CODE == "0404"& FINANCIAL_YEAR!="2015/2016") %>% 
  group_by(FINANCIAL_QUARTER, SECTION_NAME, SECTION_CODE) %>% 
  summarise(`Prescribed items` = sum(ITEM_COUNT),
            `Identified patients` = sum(PATIENT_COUNT),
            .groups = "drop") %>% 
...
chart_data$monthly %>% 
  filter(SECTION_CODE == "0404"& YEAR_MONTH>201603) %>% 
  rename(`Prescribed items` = ITEM_COUNT,
         `Identified patients` = PATIENT_COUNT) %>% 
  pivot_longer(cols = c(`Identified patients`,`Prescribed items`),
               names_to = "measure",
...
raw_data$quarterly %>%
  filter(SECTION_CODE == "0411"& FINANCIAL_YEAR!="2015/2016") %>% 
  group_by(FINANCIAL_QUARTER, SECTION_NAME, SECTION_CODE) %>% 
  summarise(`Prescribed items` = sum(ITEM_COUNT),
            `Identified patients` = sum(PATIENT_COUNT),
            .groups = "drop") %>% 
...
chart_data$monthly %>% 
  filter(SECTION_CODE == "0411"& YEAR_MONTH>201603) %>% 
  rename(`Prescribed items` = ITEM_COUNT,
         `Identified patients` = PATIENT_COUNT) %>% 
  pivot_longer(cols = c(`Identified patients`,`Prescribed items`),
               names_to = "measure",
...
In the 22-month period from March 2020 to December 2021, 3.57 million CNS stimulants and drugs used for ADHD items were prescribed in England. This was 71,500 or 2.05% more than the 3.49 million items expected based on historical trends. This increase in prescribed items does not fall outside of the expected range of values for the overall period. However, all three months of quarter 3 2021/22 have been above the number of items expected based on historical trends.  
