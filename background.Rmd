...
The final model was also tested using smaller 5 year age bands. This increased the number of age band levels used, and therefore the number of possible combinations with gender. Although the model using 5 year age bands had a lower AIC, this model was not selected because of evidence of overfitting. Overfitting occurs when adding many variables into a model improves the fit of the model on its training data, but makes the model less generalisable to new data. Ten year age bands were used instead to reduce both the overall complexity of the model and the risk of overfitting.

The model estimates the expected number of items prescribed per month based on pre-covid trends, for every combination of age band and gender. These estimates were added together to get an estimate of the total items expected. Prediction intervals at the 95% and 99% level were created. Prediction intervals were calculated based on the total items expected, and not on the individual predictions by age band and gender. The total expected items were then compared to the actual items prescribed in each month.
...
From 2021/2022 we have amended the annual publication  to show patients broken down by age band and gender, previous publications included a break down by age band and a break down by sex but not combined a breakdown. Patients broken down by age band alone remain in the statistical tables but not the statistical summaries.
